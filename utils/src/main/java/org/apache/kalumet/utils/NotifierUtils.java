...
public class NotifierUtils {
    ...
    public static void waitAndNotify(Environment environment) {
        ...
        LOGGER.debug("Send e-mail to notify people for the update of the environment {} and wait the count down ({} minute(s)).", environment.getName(), notifiers.getCountdown());
        LOGGER.debug("Construct the e-mail content.");
        LOGGER.debug("Load the e-mail template.");
        InputStreamReader notifyTemplate = new InputStreamReader(NotifierUtils.class.getResourceAsStream("/templates/notifier.html"));
