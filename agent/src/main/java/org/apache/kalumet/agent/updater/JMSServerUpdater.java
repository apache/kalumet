...
public class JMSServerUpdater {
    ...
    */
   public static void update(Environment environment, J2EEApplicationServer server, JMSServer jmsServer, UpdateLog updateLog) throws UpdateException {
      ...
      if (!jmsServer.isActive()) {
         ...
         updateLog.addUpdateMessage(new UpdateMessage("info", "JMS server " + jmsServer.getName() + " is inactive, so not updated"));
