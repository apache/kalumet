...
public class JNDIBindingUpdater {
    ...
    public static void update(Environment environment, J2EEApplicationServer server, JNDIBinding jndiBinding, UpdateLog updateLog) throws UpdateException {
        ...
        if (!jndiBinding.isActive()) {
            ...
            updateLog.addUpdateMessage(new UpdateMessage("info", "JNDI binding " + jndiBinding.getName() + " is inactive, so not updated"));
            EventUtils.post(environment, "UPDATE", "JNDI binding " + jndiBinding.getName() + " is inactive, so not updated");
            return;
        }
        ...
        try {
            if (controller.isJNDIBindingDeployed(jndiBinding.getName())) {
                ...
                updateLog.setStatus("Update performed");
                updateLog.setUpdated(true);
                updateLog.addUpdateMessage(new UpdateMessage("info", "JNDI binding " + jndiBinding.getName() + " deployed"));
    ...
    public static boolean check(String environmentName, String serverName, String jndiBindingName) throws KalumetException {
        ...
        if (jndiBinding == null) {
            LOGGER.error("JNDI binding {} is not found in J2EE application server {}", jndiBindingName, applicationServer.getName());
            throw new KalumetException("JNDI binding " + jndiBindingName + " is not found in J2EE application server " + applicationServer.getName());
        }
