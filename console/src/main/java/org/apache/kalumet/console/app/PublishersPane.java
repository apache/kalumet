 ...
 */
public class PublishersPane extends ContentPane {
    ...
    private ActionListener editDestination = new ActionListener() {
        public void actionPerformed(ActionEvent event) {
            ...
            // get field
            TextField addressField = (TextField) PublishersPane.this.getComponent("publisherdestination_" + parent.getEnvironmentName() + "_" + (String) scopeField.getSelectedItem() + "_" + address);
            String addressFieldValue = addressField.getText();
            // check the field value
            if (addressFieldValue == null || addressFieldValue.trim().length() < 1) {
                KalumetConsoleApplication.getApplication().getLogPane().addWarning(Messages.getString("publisher.destination.mandatory"), getEnvironmentWindow().getEnvironmentName());
                return;
            }
            // if the user change the destination address, check if the address is
            // not already in use
            if (!address.equals(addressFieldValue)) {
                if (parent.getEnvironment().getPublisher((String) scopeField.getSelectedItem()).getDestination(addressFieldValue) != null) {
                    KalumetConsoleApplication.getApplication().getLogPane().addWarning(Messages.getString("publisher.destination.exists"), getEnvironmentWindow().getEnvironmentName());
                    return;
                }
            }
            // looking for the destination object
            Destination destination = parent.getEnvironment().getPublisher((String) scopeField.getSelectedItem()).getDestination(address);
            if (destination == null) {
                KalumetConsoleApplication.getApplication().getLogPane().addWarning(Messages.getString("publisher.destination.notfound"), getEnvironmentWindow().getEnvironmentName());
                return;
            }
            // add a change event
    ...
    private ActionListener deleteDestination = new ActionListener() {
        public void actionPerformed(ActionEvent event) {
            ...
            String address = event.getActionCommand();
            // looking for the destination object
            final Destination destination = parent.getEnvironment().getPublisher((String) scopeField.getSelectedItem()).getDestination(address);
            if (destination == null) {
                KalumetConsoleApplication.getApplication().getLogPane().addWarning(Messages.getString("publisher.destination.notfound"), getEnvironmentWindow().getEnvironmentName());
                return;
            }
            ...
            KalumetConsoleApplication.getApplication().getDefaultWindow().getContent().add(new ConfirmWindow(new ActionListener() {
                public void actionPerformed(ActionEvent event) {
                    ...
                    parent.getEnvironment().getPublisher((String) scopeField.getSelectedItem()).getDestinations().remove(destination);
                    // add a change event
    ...
    private ActionListener deletePublisher = new ActionListener() {
        public void actionPerformed(ActionEvent event) {
            ...
            if (publisher == null) {
                KalumetConsoleApplication.getApplication().getLogPane().addWarning(Messages.getString("publisher.notfound"), getEnvironmentWindow().getEnvironmentName());
                return;
            }
            // display confirm window
            KalumetConsoleApplication.getApplication().getDefaultWindow().getContent().add(new ConfirmWindow(new ActionListener() {
    ...
    private ActionListener editPublisher = new ActionListener() {
        public void actionPerformed(ActionEvent event) {
            if (KalumetConsoleApplication.getApplication().getDefaultWindow().getContent().getComponent("publisherwindow_" + parent.getEnvironmentName() + "_" + event.getActionCommand()) == null) {
                KalumetConsoleApplication.getApplication().getDefaultWindow().getContent().add(new PublisherWindow(PublishersPane.this, (String) scopeField.getSelectedItem()));
            }
        }
    };
    // create publisher
    private ActionListener createPublisher = new ActionListener() {
        public void actionPerformed(ActionEvent event) {
            KalumetConsoleApplication.getApplication().getDefaultWindow().getContent().add(new PublisherWindow(PublishersPane.this, null));
        }
    };
    // copy publisher
    ...
    private ActionListener copyDestination = new ActionListener() {
        public void actionPerformed(ActionEvent event) {
            // looking for the destination object
            Destination destination = parent.getEnvironment().getPublisher((String) scopeField.getSelectedItem()).getDestination(event.getActionCommand());
            if (destination == null) {
                return;
            }
            try {
    ...
    public PublishersPane(EnvironmentWindow parent) {
        ...
        for (Iterator publisherIterator = parent.getEnvironment().getPublishers().iterator(); publisherIterator.hasNext(); ) {
    ...
    public void update() {
        ...
        for (Iterator destinationIterator = parent.getEnvironment().getPublisher(publisherMailhost).getDestinations().iterator(); destinationIterator.hasNext(); ) {
            ...
            destinationAddress.setId("publisherdestination_" + parent.getEnvironmentName() + "_" + publisherMailhost + "_" + destination.getAddress());
