 ...
 * Admin user window.
 */
public class AdminUserWindow extends WindowPane {
    ...
    private ActionListener applyActionListener = new ActionListener() {
        public void actionPerformed(ActionEvent event) {
            ...
            if (userNewId.length() < 1) {
                KalumetConsoleApplication.getApplication().getDefaultWindow().getContent().add(new ErrorWindow(Messages.getString("AdminUserWindow.Error.Save"), Messages.getString("AdminUserWindow.Error.EmptyUserId")));
                return;
            }
            // password must match confirm password is not empty
            if (userPassword.length() > 0 && !userPassword.equals(userConfirmPassword)) {
                KalumetConsoleApplication.getApplication().getDefaultWindow().getContent().add(new ErrorWindow(Messages.getString("AdminUserWindow.Error.Save"), Messages.getString("AdminUserWindow.Error.PasswordMatch")));
                return;
            }
            // crypt password
            if (userPassword.length() > 0) {
                try {
                    ...
                    KalumetConsoleApplication.getApplication().getDefaultWindow().getContent().add(new ErrorWindow(Messages.getString("AdminUserWindow.Error.PasswordCrypt"), e.getMessage()));
                    return;
                }
            }
            // if the admin change the user id or if it's a new user, check if the
            // id is not already used
            if (userId == null || userId.trim().length() < 1 || ((userId != null) && (userId.trim().length() > 0) && (!userNewId.equals(userId)))) {
                for (Iterator userIterator = parent.getUsers().iterator(); userIterator.hasNext(); ) {
                    User current = (User) userIterator.next();
                    if (current.getId().equals(userNewId)) {
                        KalumetConsoleApplication.getApplication().getDefaultWindow().getContent().add(new ErrorWindow(Messages.getString("AdminUserWindow.Error.Add"), Messages.getString("AdminUserWindow.Error.UserIdAlreadyExists")));
                        return;
                    }
                }
            }
    ...
    public AdminUserWindow(AdminUsersWindow parent, String userId) {
        ...
        if (!KalumetConsoleApplication.getApplication().getUserid().equals("admin")) {
            KalumetConsoleApplication.getApplication().getLogPane().addWarning(Messages.getString("uses.warn.restricted"));
            return;
        }
