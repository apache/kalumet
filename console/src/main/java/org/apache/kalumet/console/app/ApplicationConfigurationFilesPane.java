 ...
 */
public class ApplicationConfigurationFilesPane extends ContentPane {
    ...
    class StatusThread extends Thread {
        ...
        public void run() {
            try {
                ...
                boolean uptodate = client.check(parent.getParentPane().getEnvironmentWindow().getEnvironmentName(), parent.getServerName(), parent.getApplicationName(), configurationFileName);
                if (uptodate) {
                    message = "J2EE application " + parent.getApplicationName() + " configuration file " + configurationFileName + " is up to date.";
                } else {
                    failure = true;
                    message = "J2EE application " + parent.getApplicationName() + " configuration file " + configurationFileName + " is not up to date.";
                }
            } catch (Exception e) {
                failure = true;
                message = "J2EE application " + parent.getApplicationName() + " configuration file " + configurationFileName + " status check failed: " + e.getMessage();
            } finally {
                ended = true;
            }
        }
    ...
    class UpdateThread extends Thread {
        ...
        public void run() {
            try {
                ...
                // call the web service
                ConfigurationFileClient client = new ConfigurationFileClient(agent.getHostname(), agent.getPort());
                client.update(parent.getParentPane().getEnvironmentWindow().getEnvironmentName(), parent.getServerName(), parent.getApplicationName(), configurationFileName, false);
            } catch (Exception e) {
                failure = true;
                message = "J2EE application " + parent.getApplicationName() + " configuration file " + configurationFileName + " update failed: " + e.getMessage();
            } finally {
                ended = true;
            }
        }
    ...
    private ActionListener toggleActive = new ActionListener() {
        public void actionPerformed(ActionEvent event) {
            ...
            if (configurationFile == null) {
                KalumetConsoleApplication.getApplication().getLogPane().addWarning(Messages.getString("configurationfile.notfound"), parent.getParentPane().getEnvironmentWindow().getEnvironmentName());
                return;
            }
            // change the state and add a change event
            if (configurationFile.isActive()) {
                configurationFile.setActive(false);
                parent.getParentPane().getEnvironmentWindow().getChangeEvents().add("Disable J2EE application configuration file " + configurationFile.getName());
            } else {
                configurationFile.setActive(true);
                parent.getParentPane().getEnvironmentWindow().getChangeEvents().add("Enable J2EE application configuration file " + configurationFile.getName());
            }
    ...
    private ActionListener toggleBlocker = new ActionListener() {
        public void actionPerformed(ActionEvent event) {
            ...
            if (configurationFile.isBlocker()) {
                ...
                parent.getParentPane().getEnvironmentWindow().getChangeEvents().add("Set not blocker for J2EE application configuration file " + configurationFile.getName());
            } else {
                configurationFile.setBlocker(true);
                parent.getParentPane().getEnvironmentWindow().getChangeEvents().add("Set blocker for J2EE application configuration file " + configurationFile.getName());
            }
            // change the updated flag
            parent.getParentPane().getEnvironmentWindow().setUpdated(true);
            // update the journal log tab pane
    ...
    private ActionListener delete = new ActionListener() {
        public void actionPerformed(ActionEvent event) {
            ...
            // looking for the configuration file object
            final ConfigurationFile configurationFile = parent.getApplication().getConfigurationFile(event.getActionCommand());
            if (configurationFile == null) {
                KalumetConsoleApplication.getApplication().getLogPane().addWarning(Messages.getString("configurationfile.notfound"), parent.getParentPane().getEnvironmentWindow().getEnvironmentName());
                return;
            }
            ...
            KalumetConsoleApplication.getApplication().getDefaultWindow().getContent().add(new ConfirmWindow(new ActionListener() {
                public void actionPerformed(ActionEvent event) {
                    ...
                    // add a change event
                    parent.getParentPane().getEnvironmentWindow().getChangeEvents().add("Delete J2EE application configuration file " + configurationFile.getName());
                    // change the updated flag
                    parent.getParentPane().getEnvironmentWindow().setUpdated(true);
                    // update the journal log tab pane
                    parent.getParentPane().getEnvironmentWindow().updateJournalPane();
    ...
    private ActionListener edit = new ActionListener() {
        public void actionPerformed(ActionEvent event) {
            if (KalumetConsoleApplication.getApplication().getDefaultWindow().getContent().getComponent(
                    "configurationfilewindow_" + parent.getParentPane().getEnvironmentWindow().getEnvironmentName() + "_" + parent.getServerName() + "_" + parent.getApplicationName() + "_" + event.getActionCommand()) == null) {
                KalumetConsoleApplication.getApplication().getDefaultWindow().getContent().add(new ApplicationConfigurationFileWindow(ApplicationConfigurationFilesPane.this, event.getActionCommand()));
            }
        }
    };
    // create
    private ActionListener create = new ActionListener() {
        public void actionPerformed(ActionEvent event) {
            KalumetConsoleApplication.getApplication().getDefaultWindow().getContent().add(new ApplicationConfigurationFileWindow(ApplicationConfigurationFilesPane.this, null));
        }
    };
    // copy
    ...
    private ActionListener down = new ActionListener() {
        public void actionPerformed(ActionEvent event) {
            ...
            // do nothing
            if (index == -1 || index == parent.getApplication().getConfigurationFiles().size() - 1 || parent.getApplication().getConfigurationFiles().size() < 2) {
                return;
            }
    ...
    private ActionListener status = new ActionListener() {
        public void actionPerformed(ActionEvent event) {
            ...
            if (parent.getParentPane().getEnvironmentWindow().isUpdated()) {
                KalumetConsoleApplication.getApplication().getLogPane().addError(Messages.getString("environment.notsaved"), parent.getParentPane().getEnvironmentWindow().getEnvironmentName());
                return;
            }
            String configurationFileName = event.getActionCommand();
            // add a message into the log pane and the journal
            KalumetConsoleApplication.getApplication().getLogPane().addInfo("J2EE application " + parent.getApplicationName() + " configuration file " + configurationFileName + " status check in progress...", parent.getParentPane().getEnvironmentWindow().getEnvironmentName());
            parent.getParentPane().getEnvironmentWindow().getChangeEvents().add("J2EE application " + parent.getApplicationName() + " configuration file " + configurationFileName + " status check requested.");
                ...
                public void run() {
                    if (statusThread.ended) {
                        if (statusThread.failure) {
                            KalumetConsoleApplication.getApplication().getLogPane().addWarning(statusThread.message, parent.getParentPane().getEnvironmentWindow().getEnvironmentName());
                        } else {
                            KalumetConsoleApplication.getApplication().getLogPane().addInfo(statusThread.message, parent.getParentPane().getEnvironmentWindow().getEnvironmentName());
                        }
                    } else {
        ...
        public void actionPerformed(ActionEvent event) {
            ...
            KalumetConsoleApplication.getApplication().getDefaultWindow().getContent().add(new ConfirmWindow(new ActionListener() {
                public void actionPerformed(ActionEvent event) {
                    ...
                    KalumetConsoleApplication.getApplication().getLogPane().addInfo("J2EE application " + parent.getApplicationName() + " configuration file " + configurationFileName + " update in progress...", parent.getParentPane().getEnvironmentWindow().getEnvironmentName());
                    parent.getParentPane().getEnvironmentWindow().getChangeEvents().add("J2EE application " + parent.getApplicationName() + " configuration file " + configurationFileName + " update requested.");
                        ...
                        public void run() {
                            if (updateThread.ended) {
                                if (updateThread.failure) {
                                    ...
                                    KalumetConsoleApplication.getApplication().getLogPane().addConfirm("J2EE application " + parent.getApplicationName() + " configuration file " + configurationFileName + " updated.", parent.getParentPane().getEnvironmentWindow().getEnvironmentName());
                                    parent.getParentPane().getEnvironmentWindow().getChangeEvents().add("J2EE application " + parent.getApplicationName() + " configuration file " + configurationFileName + " updated.");
                                }
                            } else {
                                KalumetConsoleApplication.getApplication().enqueueTask(KalumetConsoleApplication.getApplication().getTaskQueue(), this);
                            }
    ...
    public void update() {
        ...
        for (Iterator configurationFileIterator = parent.getApplication().getConfigurationFiles().iterator(); configurationFileIterator.hasNext(); ) {
