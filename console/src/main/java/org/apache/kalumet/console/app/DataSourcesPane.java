 ...
 */
public class DataSourcesPane extends ContentPane {
    ...
    class StatusThread extends Thread {
        ...
        public void run() {
            try {
                ...
                boolean ok = client.check(parent.getEnvironmentName(), (String) scopeSelectField.getSelectedItem(), datasourceName);
                if (ok) {
    ...
    private ActionListener edit = new ActionListener() {
        public void actionPerformed(ActionEvent event) {
            ...
            // get the field
            TextField nameField = (TextField) DataSourcesPane.this.getComponent("dsname_" + parent.getEnvironmentName() + "_" + (String) scopeSelectField.getSelectedItem() + "_" + dataSourceName);
            SelectField connectionPoolField = (SelectField) DataSourcesPane.this.getComponent("dspool_" + parent.getEnvironmentName() + "_" + (String) scopeSelectField.getSelectedItem() + "_" + dataSourceName);
            // check if the user has selected a pool in the select field
            if (connectionPoolField.getSelectedIndex() < 0) {
                KalumetConsoleApplication.getApplication().getLogPane().addWarning(Messages.getString("datasource.noconnectionpool"), getEnvironmentWindow().getEnvironmentName());
                return;
            }
            // get values
            ...
            if (nameFieldValue == null || nameFieldValue.trim().length() < 1 || connectionPoolFieldValue == null || connectionPoolFieldValue.trim().length() < 1) {
                KalumetConsoleApplication.getApplication().getLogPane().addWarning(Messages.getString("datasource.mandatory"), getEnvironmentWindow().getEnvironmentName());
                return;
            }
            // if the user change the data source name, check if the data source
            // name is not already in use
            if (!dataSourceName.equals(nameFieldValue)) {
                if (getEnvironmentWindow().getEnvironment().getJ2EEApplicationServers().getJ2EEApplicationServer((String) scopeSelectField.getSelectedItem()).getJDBCDataSource(nameFieldValue) != null) {
                    KalumetConsoleApplication.getApplication().getLogPane().addWarning(Messages.getString("datasource.exists"), getEnvironmentWindow().getEnvironmentName());
                    return;
                }
            }
            // looking for the data source object
            JDBCDataSource dataSource = parent.getEnvironment().getJ2EEApplicationServers().getJ2EEApplicationServer((String) scopeSelectField.getSelectedItem()).getJDBCDataSource(dataSourceName);
            if (dataSource == null) {
                KalumetConsoleApplication.getApplication().getLogPane().addWarning(Messages.getString("datasource.notfound"), getEnvironmentWindow().getEnvironmentName());
                return;
            }
            // add a change event
    ...
    private ActionListener create = new ActionListener() {
        public void actionPerformed(ActionEvent event) {
            ...
            // mandatory field
            if (newConnectionPoolFieldValue == null || newConnectionPoolFieldValue.trim().length() < 1 || newNameFieldValue == null || newNameFieldValue.trim().length() < 1) {
                KalumetConsoleApplication.getApplication().getLogPane().addWarning(Messages.getString("datasource.mandatory"), getEnvironmentWindow().getEnvironmentName());
                return;
            }
            ...
            // add the data source
            try {
                parent.getEnvironment().getJ2EEApplicationServers().getJ2EEApplicationServer((String) scopeSelectField.getSelectedItem()).addJDBCDataSource(dataSource);
            } catch (Exception e) {
                KalumetConsoleApplication.getApplication().getLogPane().addWarning(Messages.getString("datasource.exists"), getEnvironmentWindow().getEnvironmentName());
                return;
            }
    ...
    private ActionListener delete = new ActionListener() {
        public void actionPerformed(ActionEvent event) {
            ...
            final JDBCDataSource dataSource = parent.getEnvironment().getJ2EEApplicationServers().getJ2EEApplicationServer((String) scopeSelectField.getSelectedItem()).getJDBCDataSource(event.getActionCommand());
            if (dataSource == null) {
                KalumetConsoleApplication.getApplication().getLogPane().addWarning(Messages.getString("datasource.notfound"), getEnvironmentWindow().getEnvironmentName());
                return;
            }
            ...
            KalumetConsoleApplication.getApplication().getDefaultWindow().getContent().add(new ConfirmWindow(new ActionListener() {
                public void actionPerformed(ActionEvent event) {
                    ...
                    parent.getEnvironment().getJ2EEApplicationServers().getJ2EEApplicationServer((String) scopeSelectField.getSelectedItem()).getJDBCDataSources().remove(dataSource);
                    // add a change event
    ...
    private ActionListener status = new ActionListener() {
        public void actionPerformed(ActionEvent event) {
            ...
            KalumetConsoleApplication.getApplication().getLogPane().addInfo("JDBC data source " + datasourceName + " status check in progress...", parent.getEnvironmentName());
        ...
        public void actionPerformed(ActionEvent event) {
            ...
            KalumetConsoleApplication.getApplication().getDefaultWindow().getContent().add(new ConfirmWindow(new ActionListener() {
                public void actionPerformed(ActionEvent event) {
                    ...
                    KalumetConsoleApplication.getApplication().getLogPane().addInfo("JDBC data source " + datasourceName + " update in progress...", parent.getEnvironmentName());
                        ...
                        public void run() {
                            if (updateThread.ended) {
                                if (updateThread.failure) {
                                    ...
                                    KalumetConsoleApplication.getApplication().getLogPane().addConfirm("JDBC data source " + datasourceName + " updated.", parent.getEnvironmentName());
    ...
    private ActionListener toggleActive = new ActionListener() {
        public void actionPerformed(ActionEvent event) {
            ...
            // looking for the data source object
            JDBCDataSource dataSource = parent.getEnvironment().getJ2EEApplicationServers().getJ2EEApplicationServer((String) scopeSelectField.getSelectedItem()).getJDBCDataSource(event.getActionCommand());
            if (dataSource == null) {
                KalumetConsoleApplication.getApplication().getLogPane().addWarning(Messages.getString("datasource.notfound"), getEnvironmentWindow().getEnvironmentName());
                return;
            }
    ...
    public void update() {
        ...
        for (Iterator jdbcDataSourceIterator = parent.getEnvironment().getJ2EEApplicationServers().getJ2EEApplicationServer(applicationServerName).getJDBCDataSources().iterator(); jdbcDataSourceIterator.hasNext(); ) {
            ...
            dataSourceName.setId("dsname_" + parent.getEnvironmentName() + "_" + applicationServerName + "_" + dataSource.getName());
            ...
            dataSourcePool.setId("dspool_" + parent.getEnvironmentName() + "_" + applicationServerName + "_" + dataSource.getName());
            DefaultListModel listModel = (DefaultListModel) dataSourcePool.getModel();
            listModel.removeAll();
            for (Iterator poolIterator = parent.getEnvironment().getJ2EEApplicationServers().getJ2EEApplicationServer(applicationServerName).getJDBCConnectionPools().iterator(); poolIterator.hasNext(); ) {
