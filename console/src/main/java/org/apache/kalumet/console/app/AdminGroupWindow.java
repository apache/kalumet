 ...
 * Admin group window.
 */
public class AdminGroupWindow extends WindowPane {
    ...
    private ActionListener refreshActionListener = new ActionListener() {
        public void actionPerformed(ActionEvent event) {
            ...
            update();
            KalumetConsoleApplication.getApplication().getLogPane().addInfo(groupId + " " + Messages.getString("reloaded"));
        }
    };
    ...
    private ActionListener addMemberActionListener = new ActionListener() {
        public void actionPerformed(ActionEvent event) {
            ...
            if (found == null) {
                KalumetConsoleApplication.getApplication().getLogPane().addWarning(Messages.getString("group.warn.user.notfound"));
                return;
            }
            try {
                group.addUser(found);
            } catch (Exception e) {
                KalumetConsoleApplication.getApplication().getLogPane().addWarning(Messages.getString("group.warn.user.alreadymember"));
                return;
            }
            // update view
            update();
    ...
    private ActionListener deleteMemberActionListener = new ActionListener() {
        public void actionPerformed(ActionEvent event) {
            ...
            if (membersBox.getSelectedValue() == null) {
                KalumetConsoleApplication.getApplication().getDefaultWindow().getContent().add(new ErrorWindow(Messages.getString("AdminGroupWindow.Error.Delete"), Messages.getString("AdminGroupWindow.Error.Delete.Select")));
                return;
            }
            ...
            if (found == null) {
                KalumetConsoleApplication.getApplication().getDefaultWindow().getContent().add(new ErrorWindow(Messages.getString("AdminGroupWindow.Error.Delete"), Messages.getString("AdminGroupWindow.Error.Delete.NotFound")));
                return;
            }
    ...
    private ActionListener applyActionListener = new ActionListener() {
        public void actionPerformed(ActionEvent event) {
            ...
            if (groupId == null || groupId.trim().length() < 1 || ((groupId != null) && (groupId.trim().length() > 0) && (!groupNewId.equals(groupId)))) {
                for (Iterator groupIterator = parent.getGroups().iterator(); groupIterator.hasNext(); ) {
                    Group current = (Group) groupIterator.next();
                    if (current.getId().equals(groupNewId)) {
                        KalumetConsoleApplication.getApplication().getDefaultWindow().getContent().add(new ErrorWindow(Messages.getString("AdminGroupWindow.Error.Save"), Messages.getString("AdminGroupWindow.Error.GroupIdAlreadyExists")));
                        return;
                    }
                }
            }
    ...
    public AdminGroupWindow(AdminGroupsWindow parent, String groupId) {
        ...
        if (!KalumetConsoleApplication.getApplication().getUserid().equals("admin")) {
            KalumetConsoleApplication.getApplication().getLogPane().addWarning(Messages.getString("groups.warn.restricted"));
            return;
        }
