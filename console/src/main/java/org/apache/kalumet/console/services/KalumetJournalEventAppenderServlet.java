 ...
 */
public class KalumetJournalEventAppenderServlet extends HttpServlet {
    ...
    public void doGet(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
        ...
        if (environment == null || environment.trim().length() < 1 || author == null || author.trim().length() < 1 || severity == null || severity.trim().length() < 1 || event == null || event.trim().length() < 1) {
            throw new ServletException("The Apache Kalumet Console journal event appender needs the environment, author, severity and event HTTP parameters.");
        }
