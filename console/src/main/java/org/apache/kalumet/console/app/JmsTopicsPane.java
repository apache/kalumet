 ...
 * JMS topics pane.
 */
public class JmsTopicsPane extends ContentPane {
    ...
    private ActionListener delete = new ActionListener() {
        public void actionPerformed(ActionEvent event) {
            ...
            if (jmsTopic == null) {
                KalumetConsoleApplication.getApplication().getLogPane().addWarning(Messages.getString("jmstopic.notfound"), getEnvironmentWindow().getEnvironmentName());
                return;
            }
    ...
    private ActionListener edit = new ActionListener() {
        public void actionPerformed(ActionEvent event) {
            ...
            // get the field
            TextField topicNameField = (TextField) JmsTopicsPane.this.getComponent("jmsTopicNameField_" + parent.getEnvironmentWindow().getEnvironmentName() + "_" + parent.getServerName() + "_" + parent.getJMSServerName() + "_"
                    + event.getActionCommand());
            // get the field value
            String topicNameFieldValue = topicNameField.getText();
            // check field
            if (topicNameFieldValue == null || topicNameFieldValue.trim().length() < 1) {
                KalumetConsoleApplication.getApplication().getLogPane().addWarning(Messages.getString("jmstopic.mandatory"), getEnvironmentWindow().getEnvironmentName());
                return;
            }
            // looking for the JMS topic object
            ...
            if (!topicNameFieldValue.equals(jmsTopic.getName())) {
                if (parent.getJMSServer().getJMSTopic(topicNameFieldValue) != null) {
                    KalumetConsoleApplication.getApplication().getLogPane().addWarning(Messages.getString("jmstopic.exists"), getEnvironmentWindow().getEnvironmentName());
                    return;
                }
            }
            // add a change event
    ...
    public void update() {
        ...
        for (Iterator topicIterator = parent.getJMSServer().getJMSTopics().iterator(); topicIterator.hasNext(); ) {
            ...
            nameField.setId("topicname_" + parent.getEnvironmentWindow().getEnvironmentName() + "_" + parent.getServerName() + "_" + parent.getJMSServerName() + "_" + topic.getName());
