 ...
 */
public class ApplicationArchivesPane extends ContentPane {
    ...
    class StatusThread extends Thread {
        ...
        public void run() {
            try {
                ...
                boolean uptodate = client.check(parent.getParentPane().getEnvironmentWindow().getEnvironmentName(), parent.getServerName(), parent.getApplicationName(), archiveName);
                if (uptodate) {
                    message = "J2EE application " + parent.getApplicationName() + " archive " + archiveName + " is up to date.";
                } else {
                    failure = true;
                    message = "J2EE application " + parent.getApplicationName() + " archive " + archiveName + " is not up to date.";
                }
            } catch (Exception e) {
                failure = true;
                message = "J2EE application " + parent.getApplicationName() + " archive " + archiveName + " status check failed: " + e.getMessage();
            } finally {
                ended = true;
            }
        }
    ...
    class UpdateThread extends Thread {
        ...
        public void run() {
            try {
                ...
                // call the web service
                ArchiveClient client = new ArchiveClient(agent.getHostname(), agent.getPort());
                client.update(parent.getParentPane().getEnvironmentWindow().getEnvironmentName(), parent.getServerName(), parent.getApplicationName(), archiveName, false);
            } catch (Exception e) {
                failure = true;
                message = "J2EE application " + parent.getApplicationName() + " archive " + archiveName + " update failed: " + e.getMessage();
            } finally {
                ended = true;
            }
        }
    ...
    private ActionListener toggleActive = new ActionListener() {
        public void actionPerformed(ActionEvent event) {
            ...
            if (!parent.getParentPane().getEnvironmentWindow().adminPermission && !parent.getParentPane().getEnvironmentWindow().jeeApplicationsPermission) {
            ...
            if (archive == null) {
                KalumetConsoleApplication.getApplication().getLogPane().addWarning(Messages.getString("archive.notfound"), parent.getParentPane().getEnvironmentWindow().getEnvironmentName());
                return;
            }
            // change the state and add change event
            if (archive.isActive()) {
                archive.setActive(false);
                parent.getParentPane().getEnvironmentWindow().getChangeEvents().add("Disable J2EE application archive " + archive.getName());
            } else {
                archive.setActive(true);
                parent.getParentPane().getEnvironmentWindow().getChangeEvents().add("Enable J2EE application archive " + archive.getName());
            }
    ...
    private ActionListener toggleBlocker = new ActionListener() {
        public void actionPerformed(ActionEvent event) {
            ...
            if (archive.isBlocker()) {
                ...
                parent.getParentPane().getEnvironmentWindow().getChangeEvents().add("Set not blocker for J2EE application archive " + archive.getName());
            } else {
                archive.setBlocker(true);
                parent.getParentPane().getEnvironmentWindow().getChangeEvents().add("Set blocker for J2EE application archive " + archive.getName());
            }
            // change the updated flag
            parent.getParentPane().getEnvironmentWindow().setUpdated(true);
            // update the journal log tab pane
        ...
        public void actionPerformed(ActionEvent event) {
            ...
            KalumetConsoleApplication.getApplication().getDefaultWindow().getContent().add(new ConfirmWindow(new ActionListener() {
                public void actionPerformed(ActionEvent event) {
                    ...
                    // add a change event
                    parent.getParentPane().getEnvironmentWindow().getChangeEvents().add("Delete J2EE application archive " + archive.getName());
                    // change the updated flag
                    parent.getParentPane().getEnvironmentWindow().setUpdated(true);
                    // update the journal log tab pane
                    parent.getParentPane().getEnvironmentWindow().updateJournalPane();
    ...
    private ActionListener status = new ActionListener() {
        public void actionPerformed(ActionEvent event) {
            ...
            KalumetConsoleApplication.getApplication().getLogPane().addInfo("J2EE application " + parent.getApplicationName() + " archive " + archiveName + " status check in progress...", parent.getParentPane().getEnvironmentWindow().getEnvironmentName());
            parent.getParentPane().getEnvironmentWindow().getChangeEvents().add("J2EE application " + parent.getApplicationName() + " archive " + archiveName + " check requested.");
    ...
    private ActionListener update = new ActionListener() {
        public void actionPerformed(ActionEvent event) {
            ...
            // check if the user can do it
            if (!parent.getParentPane().getEnvironmentWindow().adminPermission && !parent.getParentPane().getEnvironmentWindow().jeeApplicationsUpdatePermission) {
                KalumetConsoleApplication.getApplication().getLogPane().addWarning(Messages.getString("action.restricted"), parent.getParentPane().getEnvironmentWindow().getEnvironmentName());
                return;
            }
            ...
            KalumetConsoleApplication.getApplication().getDefaultWindow().getContent().add(new ConfirmWindow(new ActionListener() {
                public void actionPerformed(ActionEvent event) {
                    ...
                    KalumetConsoleApplication.getApplication().getLogPane().addInfo("J2EE application " + parent.getApplicationName() + " archive " + archiveName + " update in progress...", parent.getParentPane().getEnvironmentWindow().getEnvironmentName());
                    parent.getParentPane().getEnvironmentWindow().getChangeEvents().add("J2EE application " + parent.getApplicationName() + " archive " + archiveName + " update requested.");
                        ...
                        public void run() {
                            if (updateThread.ended) {
                                if (updateThread.failure) {
                                    ...
                                    KalumetConsoleApplication.getApplication().getLogPane().addConfirm("J2EE application " + parent.getApplicationName() + " archive " + archiveName + " updated.", parent.getParentPane().getEnvironmentWindow().getEnvironmentName());
                                    parent.getParentPane().getEnvironmentWindow().getChangeEvents().add("J2EE application " + parent.getApplicationName() + " archive " + archiveName + " updated");
                                }
                            } else {
                                KalumetConsoleApplication.getApplication().enqueueTask(KalumetConsoleApplication.getApplication().getTaskQueue(), this);
                            }
    ...
    private ActionListener edit = new ActionListener() {
        public void actionPerformed(ActionEvent event) {
            if (KalumetConsoleApplication.getApplication().getDefaultWindow().getContent().getComponent(
                    "archivewindow_" + parent.getParentPane().getEnvironmentWindow().getEnvironmentName() + "_" + parent.getServerName() + "_" + parent.getApplicationName() + "_" + event.getActionCommand()) == null) {
                KalumetConsoleApplication.getApplication().getDefaultWindow().getContent().add(new ApplicationArchiveWindow(ApplicationArchivesPane.this, event.getActionCommand()));
            }
        }
    };
    // create
    private ActionListener create = new ActionListener() {
        public void actionPerformed(ActionEvent event) {
            KalumetConsoleApplication.getApplication().getDefaultWindow().getContent().add(new ApplicationArchiveWindow(ApplicationArchivesPane.this, null));
        }
    };
    // copy
    ...
    private ActionListener down = new ActionListener() {
        public void actionPerformed(ActionEvent event) {
            ...
            // do nothing
            if (index == -1 || index == parent.getApplication().getArchives().size() - 1 || parent.getApplication().getArchives().size() < 2) {
                return;
            }
