 ...
 */
public class SoftwareConfigurationFileWindow extends WindowPane {
    ...
    class UpdateThread extends Thread {
        ...
        public void run() {
            try {
                ...
                client.updateConfigurationFile(parent.getParentPane().getEnvironmentWindow().getEnvironmentName(), parent.getName(), name, false);
            } catch (Exception e) {
                failure = true;
        ...
        public void actionPerformed(ActionEvent event) {
            ...
            KalumetConsoleApplication.getApplication().getDefaultWindow().getContent().add(new ConfirmWindow(new ActionListener() {
                public void actionPerformed(ActionEvent event) {
                    ...
                    // add a change event
                    parent.getParentPane().getEnvironmentWindow().getChangeEvents().add("Delete software " + parent.getName() + " configuration file " + configurationFile.getName());
    ...
    private ActionListener apply = new ActionListener() {
        public void actionPerformed(ActionEvent event) {
            ...
            if (nameFieldValue == null || nameFieldValue.trim().length() < 1 || uriFieldValue == null || uriFieldValue.trim().length() < 1) {
                KalumetConsoleApplication.getApplication().getLogPane().addWarning(Messages.getString("configurationfile.mandatory"), getEnvironmentWindow().getEnvironmentName());
                return;
            }
            ...
            // add a change event
            if (name != null) {
                parent.getParentPane().getEnvironmentWindow().getChangeEvents().add("Change software " + parent.getName() + " configuration file " + configurationFile.getName());
            }
            ...
            if (name == null) {
                try {
                    ...
                    parent.getParentPane().getEnvironmentWindow().getChangeEvents().add("Add software " + parent.getName() + " configuration file " + configurationFile.getName());
                } catch (Exception e) {
                    KalumetConsoleApplication.getApplication().getLogPane().addWarning(Messages.getString("software.component.exists"), getEnvironmentWindow().getEnvironmentName());
                    return;
                }
            ...
            setId("softwareconfigurationfilewindow_" + parent.getParentPane().getEnvironmentWindow().getEnvironmentName() + "_" + parent.getName() + "_" + configurationFile.getName());
    ...
    private ActionListener deleteMapping = new ActionListener() {
        public void actionPerformed(ActionEvent event) {
            ...
            if (mapping == null) {
                KalumetConsoleApplication.getApplication().getLogPane().addWarning(Messages.getString("mapping.notfound"), getEnvironmentWindow().getEnvironmentName());
                return;
            }
            // delete the mapping object
            configurationFile.getMappings().remove(mapping);
            // add a change event
            parent.getParentPane().getEnvironmentWindow().getChangeEvents().add("Delete software " + parent.getName() + " configuration file " + configurationFile.getName() + " mapping " + mapping.getKey());
            // change the updated flag
            parent.getParentPane().getEnvironmentWindow().setUpdated(true);
            // update the journal log tab pane
            parent.getParentPane().getEnvironmentWindow().updateJournalPane();
    ...
    private ActionListener editMapping = new ActionListener() {
        public void actionPerformed(ActionEvent event) {
            ...
            // get fields
            TextField keyField = (TextField) SoftwareConfigurationFileWindow.this.getComponent("softwareconfigurationfilemappingkey_" + parent.getParentPane().getEnvironmentWindow().getEnvironmentName() + "_" + parent.getName() + "_" + name + "_" + event.getActionCommand());
            TextField valueField = (TextField) SoftwareConfigurationFileWindow.this.getComponent("softwareconfigurationfilemappingvalue_" + parent.getParentPane().getEnvironmentWindow().getEnvironmentName() + "_" + parent.getName() + "_" + name + "_" + event.getActionCommand());
            // get fields value
            ...
            if (keyFieldValue == null || keyFieldValue.trim().length() < 1) {
                KalumetConsoleApplication.getApplication().getLogPane().addWarning(Messages.getString("mapping.mandatory"), getEnvironmentWindow().getEnvironmentName());
                return;
            }
            ...
            if (!keyFieldValue.equals(event.getActionCommand())) {
                if (configurationFile.getMapping(keyFieldValue) != null) {
                    KalumetConsoleApplication.getApplication().getLogPane().addWarning(Messages.getString("mapping.exists"), getEnvironmentWindow().getEnvironmentName());
                    return;
                }
            }
            // looking for the mapping object
            ...
            // add a change event
            parent.getParentPane().getEnvironmentWindow().getChangeEvents().add("Change software " + parent.getName() + " configuration file " + configurationFile.getName() + " mapping " + mapping.getKey());
            // update the mapping
    ...
    private ActionListener createMapping = new ActionListener() {
        public void actionPerformed(ActionEvent event) {
            ...
            // add a change event
            parent.getParentPane().getEnvironmentWindow().getChangeEvents().add("Add software " + parent.getName() + " configuration file " + configurationFile.getName() + " mapping " + mapping.getKey());
            // change the updated flag
            parent.getParentPane().getEnvironmentWindow().setUpdated(true);
            // update the journal log tab pane
            parent.getParentPane().getEnvironmentWindow().updateJournalPane();
        ...
        public void actionPerformed(ActionEvent event) {
            ...
            KalumetConsoleApplication.getApplication().getDefaultWindow().getContent().add(new ConfirmWindow(new ActionListener() {
                public void actionPerformed(ActionEvent event) {
                    ...
                    KalumetConsoleApplication.getApplication().getLogPane().addInfo("Software " + parent.getName() + " configuration file " + name + " update in progress ...", parent.getParentPane().getEnvironmentWindow().getEnvironmentName());
                    parent.getParentPane().getEnvironmentWindow().getChangeEvents().add("Software " + parent.getName() + " configuration file " + name + " update requested.");
                        ...
                        public void run() {
                            if (updateThread.ended) {
                                if (updateThread.failure) {
                                    ...
                                    KalumetConsoleApplication.getApplication().getLogPane().addConfirm("Software " + parent.getName() + " configuration file " + name + " updated.", parent.getParentPane().getEnvironmentWindow().getEnvironmentName());
                                    parent.getParentPane().getEnvironmentWindow().getChangeEvents().add("Software " + parent.getName() + " configuration file " + name + " updated.");
                                }
                            } else {
    ...
    public SoftwareConfigurationFileWindow(SoftwareWindow parent, String name) {
        ...
        setId("softwareconfigurationfilewindow_" + parent.getParentPane().getEnvironmentWindow().getEnvironmentName() + "_" + parent.getName() + "_" + name);
    ...
    public void update() {
        ...
        for (Iterator mappingIterator = configurationFile.getMappings().iterator(); mappingIterator.hasNext(); ) {
            ...
            mappingKeyField.setId("softwareconfigurationfilemappingkey_" + parent.getParentPane().getEnvironmentWindow().getEnvironmentName() + "_" + parent.getName() + "_" + name + "_" + mapping.getKey());
            ...
            mappingValueField.setId("softwareconfigurationfilemappingvalue_" + parent.getParentPane().getEnvironmentWindow().getEnvironmentName() + "_" + parent.getName() + "_" + name + "_" + mapping.getKey());
